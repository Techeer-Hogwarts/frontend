...
export default function Project() {
        ...
        queryFn: async () => {
          ...
          const formattedTeams: Team[] = data.allTeams.map((team: any) => {
            if (team.type === 'project') {
              return {
                ...team,
                mainImages: team.mainImages?.map(
                  (img: any) => (typeof img === 'string' ? img : img.imageUrl), // ⚡ mainImages를 string[]로 변환
                ),
              } as ProjectTeam
            }
  ...
  useEffect(() => {
    if (allTeams && allTeams.allTeams) {
      setTeams((prev) => {
        ...
        const newTeams = allTeams.allTeams.filter(
  ...
  return (
    <div className="w-[1200px] mt-[3.56rem] h-auto min-h-screen">
      ...
      {/* 탭바 */}
      <TapBar options={['전체보기', '모집 중']} onSelect={handleSearch} />
